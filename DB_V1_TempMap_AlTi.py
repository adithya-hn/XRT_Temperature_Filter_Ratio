...
pair_imgs=np.loadtxt('V1_Al_Ti_Pairs_D08_10_23.dat',dtype='str')                  #al mesh and and Ti poly image pairs
r_data = (np.loadtxt("V1_Resp_data_Al_Ti_.txt")).transpose() #Response function from IDL
...
pathlib.Path("TiPNmaps").mkdir(parents=True, exist_ok=True) #TiPNmaps
...
for l in range(Length):
    ...
    if start == 1:
        ...
        hdu.header['PROG_VER']= 'DB_V1.00_Temp_Al_Ti_Oct_23'
        ...
        os.remove("{}_temp.fits".format(Fname1)) #/comp_XRT20080105_074311.0_seg.fits
        ...
        PhduTi = fits.PrimaryHDU(Kd1)
        PhduTi.writeto("{}_tempTiPN.fits".format(Fname1),overwrite=True)
        zip_file = zipfile.ZipFile("{}_tempTiPN.fits.zip".format(Fname1), 'w')
        zip_file.write("{}_tempTiPN.fits".format(Fname1), compress_type=zipfile.ZIP_DEFLATED)
        zip_file.close()
        os.replace(os.getcwd()+"/{}_tempTiPN.fits.zip".format(Fname1), os.getcwd()+'/TiPNmaps/{}_tempTiPN.fits.zip'.format(Fname1))
        os.remove("{}_tempTiPN.fits".format(Fname1))
        ...
        imageio.imwrite('Al_Ti_TempMaps/{}_temp.jpg'.format(Fname1), comb_Temp[::-1])
        ...
        f = open('V1_Al_Ti_Temp_data.dat', 'a')
        np.savetxt('V1_Al_Ti_Temp_data.dat', np.c_[
        ...
        f.close()
        fa = open('V1_Database_Al_Ti_Temp_data_10_10_23.dat', 'a')
        np.savetxt('V1_Database_Al_Ti_Temp_data_10_10_23.dat', np.c_[
